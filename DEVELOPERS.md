...
2. **プロジェクト設定** `<repo>/.zed/settings.json` にサーバの絶対パスを書く。
   これが無いと、拡張は PATH を見て、それでも見つからなければ**リリースから
   落としてくる**。つまり手元のビルドは無視され、直したはずのコードが動かない
   ように見える。開発中はここを書くのが唯一の確実な指定である。
...
4. Zed のコマンドパレットから `zed: install dev extension` を実行し、
   ...
   **これは Zed が手元の Rust を呼ぶので、Rust の無い環境では失敗する。**
   拡張を配るときの経路ではない（レジストリ経由なら、ビルド済みの wasm が
   降ってくる）ため、Rust の無い環境で試すには先にレジストリへ登録する必要が
   ある（[Issue #38](https://github.com/shutx-net/codegloss/issues/38)）。
