...
P7 で `CandleTranslator` を入れたときに実機で測った値と、P8（軽量化）・
P9（訳文の質）で足した分。**すべて実測値で、推測は書かない。**未検証のものは
最後の節にまとめてある。
...
おり、FuguMT はビームサーチ前提で評価されたモデルなのに、P7 の実装は `argmax`
だけだった。負の対数尤度を足すだけの貪欲法は、短く終わる経路を必ず好む。

もう 1 つは**入力が長すぎること**。P3 の「連続する散文行は 1 単位」により、
...
|---|---|---|---|---|---|
| A 段落＋貪欲（P7 まで） | 4850 | 151 | 95 | 45 / 100 | 1/39 |
...
- **`;` `:` 分割の下限語数。**`MIN_CLAUSE_WORDS = 4` は 1 つのコーパスで
  決めた値で、振ってはいない。
- **パディングしたバッチ推論。**フォークでマスクは入ったが（8）、バッチ推論
  そのものは実装していない。matmul 単体の表（6.5）では batch 4 と batch 8 が
  ほぼ同時間なので効きそうに見えるだけで、**実測はしていない。**
