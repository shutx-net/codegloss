...
- **`backend.rs::with_hard_breaks` は 3 つ目のフェンス判定を持ったまま。**
  ...
  **この判定はその後 `opens_or_closes_a_rendered_fence` という名前になった
  （§15.6）。寄せたのではなく、2 つ目の問いに名前を付けた。**
...
### 15.6 描画側のフェンス判定に名前を付ける

§15.4 が「寄せなかった」と書いた 3 つ目の判定
（`backend.rs::with_hard_breaks` の `line.trim_start().starts_with("```")`）に
名前を与えた。`codegloss-core` に 2 つ目の述語
`opens_or_closes_a_rendered_fence` を置き、`with_hard_breaks` はそれを呼ぶ。
**寄せたのではない**——問いは 2 つのままで、無名だった 2 つ目に名前が付いた。
裸の `starts_with("```")` は「写し忘れ」なのか「意図的な違い」なのかを読み手に
言えず、その曖昧さが #54 を生んだものだからである。

**コーパスは §14.2 と同じ 3 つ**（自リポジトリは 18bd853 の内容なので 1252
ブロック。§14.2 の 1224 は 3a1ad60 時点で、#54/#56/#57 がテストを足したぶん
増えている）。

#### 15.6.1 素朴に寄せると壊れる（引数の形が逆）

`opens_or_closes_a_fence` は**マーカーを剥がして trim 済みの行**を受け取る
契約で、`with_hard_breaks` が見ているのは**完成した gloss の行そのまま**である。
#55 でフェンスの中の字下げを保つようにしたので、gloss には字下げ付きの
フェンス行が実際に入る:

```
"  ```"
  opens_or_closes_a_fence(line)              = false   ← 素朴な置き換え
  opens_or_closes_a_fence(line.trim_start()) = true
  従来の line.trim_start().starts_with("```") = true
```

gloss の中のフェンス行を列で数えると:

| | 開きフェンス 列 0 | 開きフェンス 列 2 | 閉じフェンス 列 0 | 列 2 |
|---|---|---|---|---|
| コーパス 1（自リポジトリ 1252 ブロック） | 9 | 0 | 9 | 0 |
| コーパス 2（サードパーティ 28450 ブロック） | **2129** | **15** | 2125 | 15 |
| コーパス 3（syn 1660 ブロック） | 119 | 3 | 119 | 3 |

**サードパーティの開きフェンス 2144 件のうち 15 件が列 2 に書かれている**
（`//!   ```rust` の形。§14.3 が「案 A と案 B で答えが違う」と数えた 15 件と
同じ形である）。素朴に置き換えるとこの 15 件でフェンスを見失う。実際に
`with_hard_breaks` を `opens_or_closes_a_fence(line)`（trim 無し）に差し替えて
3 コーパスに通すと、**コーパス 2 で 15 グロス・コーパス 3 で 3 グロスが変わる**
（コーパス 1 は 0）。変わり方は doctest の全行に hard break の 2 スペースが
撒かれるというもので、thiserror の例では:
...
コーパス 2 の開きフェンスと閉じフェンスの差 4 件は、上流に閉じフェンスが無い
doctest（§13.4 の 4 件）である。

#### 15.6.2 フェンスを閉じるのは開いた文字だけ

新しい述語は CommonMark に従うので `~~~` にも当たる。**ただし
`with_hard_breaks` の側を単純な反転（toggle）のままにすると出力が変わる**：
CommonMark はフェンスを**開いた文字**でしか閉じない（0.31.2 §4.5）が、
反転はどちらの文字でも閉じてしまう。

3 コーパス 31362 グロスで、従来の実装と突き合わせた:

| | 差の出たグロス | 差の出た行 |
|---|---|---|
| 単純な反転 | コーパス 3 の **2** グロス（1・2 は 0） | **3** |
| 開いた文字だけで閉じる | **0**（3 コーパスとも） | 0 |

差が出るのは syn の ASCII 図だけで、バッククォートのフェンスの中にチルダの
行があるもの:

```
old "  ~~~~~~Path"                   new "  ~~~~~~Path  "
old "#[path = \"sys/windows.rs\"]"    new "#[path = \"sys/windows.rs\"]  "
old "                ~~~~^ ~~~~^ ~~~~" new "                ~~~~^ ~~~~^ ~~~~  "
```

**採ったのは「開いた文字だけで閉じる」ほう。**CommonMark に忠実で、かつ
3 コーパスで出力が 1 バイトも変わらない。単体テストは
`a_tilde_line_does_not_close_a_backtick_fence`。

#### 15.6.3 出力は 1 バイトも変わらない

測り方は「変更前のコミット（18bd853）を `git worktree` に出し、同じ入力を
変更前後の 2 つのバイナリに通して SHA-256 を突き合わせる」。**コーパス 1 は
このリポジトリ自身なので、変更後のツリーではなく worktree 側を入力にする**
（§14.2 と同じ注意。今回の変更はコメントを増やすので、うっかり変更後のツリーを
読むと入力ごと動く）。

- **BLOCK / UNIT / SEGMENT / SOURCE / GLOSS**: 3 コーパスとも前後で
  ハッシュが一致（1252 / 28450 / 1660 ブロック、1290 / 28052 / 1627 単位、
  2223 / 41377 / 1862 セグメント）。`codegloss-core` の公開 API は増えただけで、
  `FENCES` も `opens_or_closes_a_fence` も `PIPELINE_VERSION` も触っていない。
- **`with_hard_breaks`**: 3 コーパス 31362 グロスすべてでバイト単位に同一
  （15.6.2 の表の下の行。`backend.rs` に一時的な `#[ignore]` のテストを足して
  実物の関数を通した。テストは測ったあと外してある）。
- したがって `PIPELINE_VERSION` は**上げない**。前処理・後処理の出力が変わって
  いない（§13.7 と同じ理由で、§14.7 の裏返しには当たらない）。

#### 15.6.4 未検証

- **本物の描画結果は見ていない。**測ったのは `with_hard_breaks` が返す文字列
  までで、Zed の Markdown renderer が実際にどう描くかは
  `docs/zed-display-notes.md` の目視チェック待ち（実機を持つ人間が埋める）。
- **CommonMark の細部は写していない。**開きフェンスの情報文字列に
  バッククォートを含めない規則と、字下げ 4 列以上は fence ではなく
  indented code block になる規則は入れていない。前者は 3 コーパスの開き
  フェンス 2275 件で該当 0 件、後者は列 0 と列 2 しか無いので今日の入力では
  差が出ない。

## 16. Go に対応する（Issue #30）

Go の doc コメントは**コードフェンスを書かない**。GOROOT の 4,505 ファイル
196,406 ブロックに、バッククォートのフェンス行は **0 行**である。例は字下げで
示す。だから文法だけ配線すると、doc コメントの中のコードがそのまま散文として
エンジンへ渡る——#53 を新しい言語で作り直すことになる。

規則そのものは `go/doc/comment` が持っている（`parse.go::parseSpans`）。それを
CodeGloss の形に写すのがこの節の中身で、写し方で数字が 58.2% から 91.6% まで
動く。
...
| コーパス | 母数 |
|---|---|
| 自リポジトリ（`crates/`） | 37 ファイル 1,340 ブロック |
| この機械の registry | 8,607 ファイル 244,600 ブロック |
| 凍結コーパス（`tests/fixtures/comment-corpus.txt`） | 62 ブロック |
| GOROOT（`/usr/local/go/src`、go1.24.7） | 4,505 ファイル 196,406 ブロック |

GOROOT は `testdata/` を除き、`_test.go` を除いた `.go` 全部。`testdata` を除く
のは、そこに**わざと壊した Go ソース**が入っているからである（1,082 個の `.go`
のうち 43 個は `go/parser` が読めない）。テストファイルを除くのは Go 自身の doc の
測り方に合わせるため。

Go のブロックは `cargo run -p codegloss-parser --example extract -- --lang go`
で誰でも取り出せる。**第三者のコメントはリポジトリに置けない**ので、この節の
数字はコーパスを置く代わりに「作れる」ようにしてある（§12 と同じ）。

### 16.2 正解データは位置で取る

正解は Go 自身に出させる。`go/parser` でファイルを読み、`go/ast` の
`CommentGroup.Text()` を**行番号を保ったまま**再現し、`go/doc/comment` の
分類（`Code` / `List` / `Heading` / `Paragraph`）を絶対行番号へ戻す。突き合わせ
はテキスト一致ではなく行番号で行う——同じ 1 行が同じファイルに何度も出るので、
テキストで突き合わせると数え間違える。
...
| 種別 | 行数 |
|---|---|
| doc コメントの CODE | 10,431 |
| doc 以外のコメントの CODE | 6,193 |
| 合計 | 16,624 |
| ディレクティブ行 | 8,964 |

（単調走査で位置を決められなかった CODE 行が 14 件あり、これは正解から外して
ある。）

### 16.3 規則の候補と採点

採点は行単位。**当たり**は「規則が写した行を Go も CODE と呼ぶ」、**誤検出**は
「規則が写したが Go は CODE と呼ばない」。取りこぼしはコードが機械翻訳される
害で、誤検出は英語が原文のまま出る害である——後者は CodeGloss が無かったときと
同じで、前者は `mut pos = 0 とする。` が出る。

| 腕 | doc の当たり | 再現率 | 全コメントの当たり | 再現率 | 誤検出 |
|---|---|---|---|---|---|
| **UB ブロックコメントだけ unindent（採用）** | 9,621 | 92.2% | 15,221 | 91.6% | **1,208** |
| S unindent しない | 9,621 | 92.2% | 15,240 | 91.7% | 1,477 |
| U 全部 unindent | 5,944 | 57.0% | 9,672 | 58.2% | 1,141 |
| 採用の腕からリストの判定だけ外す | 9,621 | 92.2% | 15,253 | 91.8% | 3,821 |

腕は `parseSpans` の写しに knob を戻したもので、**採用の設定にすると出荷される
`indented_examples` と GOROOT の全ブロックで 1 件も食い違わない**ことを確かめて
から採点している（食い違い 0）。

### 16.4 unindent はブロックコメントに限る

`go/doc/comment` は本文を読む前にコメント全体の共通字下げを落とす（`unindent`）。
そのまま移植すると **58.2%** しか当たらない。CodeGloss は空の `//` 行でブロック
を割るので、gofmt 済みの例は**単独のブロック**になり、その共通接頭辞は「例だ」と
言っている当のタブだからである。

一方 `/* */` は 1 コメント 1 ブロックなので、共通字下げはファイルの字下げで
あって内容ではない。**限定して掛けると 91.6%。**

限定の代金は正解の取りこぼし **19 行**（15,240 → 15,221）、見返りは誤検出
**269 行**（1,477 → 1,208）。ただし本当の理由は 269 行ではなく、構造的な事故を
塞ぐことにある: 関数の中に書かれた星無しの `/* */` は継続行がファイルの字下げを
持つので、unindent が無いと**丸ごと例と読まれて散文の gloss が消える**。GOROOT で
数えると、unindent を落とすと **135 ブロックの gloss が丸ごと消える**（135 行。
Go はその 135 行を全部散文と呼び、コードと呼ぶ行は 1 つも含まない）。

`forceIndent`（字下げを忘れて貼られたコードを救う仕掛け）は写していない。Go 自身
が「gofmt 済みのコメントでは絶対に発火しない」と書いている救済で、当てると散文の
巻き込みが増える（16.10 の 385 行のうち、cgo を除いたぶんがこれで拾われる行）。

### 16.5 誤検出 1,208 行の中身

1,208 行のうち **1,186 行は Go が散文と呼ぶ行**である。ファイル別の上位は:

| ファイル | 行 |
|---|---|
| `crypto/internal/fips140/sha256/_asm/sha256block_amd64_avx2.go` | 222 |
| `cmd/internal/obj/s390x/asmz.go` | 154 |
| `cmd/vendor/golang.org/x/arch/arm64/arm64asm/arg.go` | 90 |
| `math/expm1.go` | 36 |
| `crypto/internal/fips140/nistec/_asm/p256_asm.go` | 31 |
| `crypto/tls/cipher_suites.go` | 27 |

無作為に読むと、上位のほとんどは**命令表・レジスタ表・ASCII の数式**
（`y0 = (e>>25) ^ (e>>11)` / `VRR-c	VECTOR MULTIPLY ODD` / `u1 = x1 * z2^2`）で、
機械翻訳に渡すより原文のまま写すほうが読み手に良い。残りは**折り返された英文の
途中の行**で、これは本物の損である（16.6）。

### 16.6 読み手の単位（ブロック）で見た損得

規則が入ると 196,406 ブロックのうち **3,190 ブロックの gloss が原文そのもの**に
なる。

| 中身 | ブロック |
|---|---|
| Go も全部 CODE と呼ぶ | 2,672 |
| コードと散文の混在 | 4 |
| **散文だけ** | **513** |
| どちらでもない | 1 |

散文だけの 513 のうち **476 は 1 行**である（表の行末コメントと ASCII の数式。
`y1 = e >> 11` / `RSL-b	CONVERT TO PACKED (from long DFP)` / `u1 = x1 * z2^2`）。
**3 行以上は 25 件**。そのうち英単語が過半を占めるものが 18 件で、読むと内訳は
機械語のリスト 2 件（`cmd/compile/internal/riscv64/ssa.go`）、HTML の断片 1 件
（pprof の `svg.go`）、数式の導出 7 件（`math/erf.go` 2 / `expm1.go` / `jn.go` /
`log.go` / `log1p.go` 2）、そして**本物の英語の段落 8 件**——
`crypto/tls/cipher_suites.go` の 5 件、
`cmd/compile/internal/ssa/_gen/S390XOps.go`、pprof の Apache ライセンス表記
2 件——である。

**4,505 ファイル 196,406 ブロックに対して、英語の段落が gloss を失うのは 8 件。**
形はほぼ全部『リストの項目の続きの段落』で、Go はコメント全体を見るので続きだと
分かるが、CodeGloss は空の `//` 行でブロックを割るので続きの段落だけが単独の
ブロックになり、先頭行が字下げされている。

### 16.7 Rust は 1 ブロックも動かない

規則は `CommentRules::Indented` の下でしか走らない。それを測った:

| コーパス | ブロック | パーサの出力が違うブロック | `units()` / `source()` が違うブロック | エンジンへ渡る文字数 |
|---|---|---|---|---|
| 自リポジトリ | 1,340 | **0** | **0** | 195,151 → 195,151 |
| registry | 244,600 | **0** | **0** | 18,671,513 → 18,671,513 |
| 凍結コーパス | 62 | — | **0** | 12,849 → 12,849 |

「前」は b52f4d4 のクレートそのもの（写しではなく、その版のパーサとコアを
そのまま依存に取ってある）。ブロックの抽出も `CommentShape` の出力も比べている。

`restore(sources()) == source()` は**この変更の前からすべてでは成り立たない**
（`join_sentences` が断片を 1 スペースで繋ぐので、文末のあとの二重空白が詰まる）。
不一致は自リポジトリ **2 → 2**、registry **805 → 805** で**増えていない**。Go は
**874 → 750 に減る**（verbatim の行は `join_sentences` を通らないため）。

### 16.8 ツールチェーンへのディレクティブ

`//go:build` / `//go:generate` の類は読み手への文ではない。判定は
`go/ast::isDirective` そのままで、`languages.rs` の `DirectiveSyntax` に置いて
ある。GOROOT では:

| | |
|---|---|
| ブロック | 203,082 → **196,406**（−6,676、3.3%） |
| 消えるブロック | 6,676 |
| 一部の行だけ失って残るブロック（散文と混在） | 44 |
| どのブロックにも入らなくなるコメント行 | **8,964** |

**この 8,964 行は `go/ast` がディレクティブと呼ぶ 8,964 行と完全に一致する**
（片側だけに出る行は 0）。`// note: this is prose`（マーカーの直後が空白）と
`//TODO: fix this`（大文字）はどちらも残る。

判定を言語に持たせたので、**Rust の `//go:build linux` は構造的にコメントのまま**
である（`DirectiveSyntax::None`）。

### 16.9 2 つ目のマーカー剥がし

`after_markers` は行を trim してから読むので、**星の無いブロックコメントの継続行**
の字下げが落ちる。それは意図した挙動で（§14 と AGENTS.md）、フェンスが「ここは
コード」と言ったあとの行を写すぶんには正しい。字下げが唯一の手掛かりである
`Indented` の下ではそうはいかないので、`after_markers_as_written` を足した。
**bool の引数にはしない**——呼び手が字下げ付きの文字列をフェンス判定に渡せてしま
い、それが #54 が消した不具合そのものだから（`opens_or_closes_a_fence` と
`opens_or_closes_a_rendered_fence` が 2 つの関数である理由と同じ）。

2 つは**行コメントでは 1 件も答えが違わない**:

| コーパス | 行コメントの行 | 食い違い | ブロックコメントの行 | 食い違い |
|---|---|---|---|---|
| 自リポジトリ | 3,270 | 0 | 0 | 0 |
| registry | 511,233 | **0** | 21,541 | 2,556 |
| GOROOT | 314,431 | **0** | 23,719 | 4,023 |

**剥がす順は開き → 閉じ → 星**（`after_markers` と同じ）。逆にすると単独行の
`*/` の答えが `""` ではなく `"/"` になる——書き手が打っていない列 0 の非空行が
生え、ブロックの共通字下げが常に空になって unindent がどこでも効かなくなる。

### 16.10 残る実損

| 何 | 量 | なぜ |
|---|---|---|
| ブロックに届かない CODE 行 | **1,018 行 / 267 ファイル** | 英数字を持たない行（`//\t}`）をパーサが落とし、連なりもそこで切れる。**切れた両側はどちらも字下げされたままなので、どちらも例として写される**——コードがエンジンに渡ることは無い。内訳は閉じ括弧だけの行 771、英数字を 1 つも持たない ASCII 図 247 |
| 届いたが当たらない CODE 行 | 385 行 | 183 行は cgo を使うファイル（28 ファイル。`cmd/cgo/internal/test/test.go` だけで 98 行）。残りは Go の `forceIndent` が拾っている行で、意図して写していない |
| cgo のプリアンブル | **90 ブロック 2,735 行** | 列 0 に書かれた C のソース。字下げ規則にも unindent にも当たらない。直すには『直前が `/* */` で直後が `import "C"`』という構文木の知識が要り、`comment_query` の外 |
| Go の `///` | 4 行 | `docblock.rs` の `LINE_MARKERS` が `///` を先に見るので 3 文字剥がされる |
| 継続行の `*` が内容であるブロック | 5 ブロック 16 行 | 同上、Rust 向けの `after_markers` の都合 |

いずれも別 Issue。**`//!` は GOROOT に 0 行。**

### 16.11 描画は未確認

字下げされた行を含む gloss は **4,688** 件。

| | 件数 |
|---|---|
| 1 行目から字下げされている | 3,273 |
| 字下げ行が散文行の**直後**に空行なしで来る | 1,410 |
| 字下げ行の前が空行 | 5 |

CommonMark では**字下げコードブロックは段落に割り込めない**ので、1,410 件は
renderer が段落の続きとして描き、字下げが潰れると**思われる**。
`backend.rs::with_hard_breaks` はフェンスの外なので末尾に 2 スペースを付ける
（＝改行は残るが字下げは失われる）。**実機の Zed でどう見えるかはこの環境では
確かめられない。**`docs/zed-display-notes.md` の欄を空のまま足してある。

翻訳単位が 0 になったブロックの code lens に原文が 1 行に潰れて出るのは**新しい
問題ではない**——Rust の doctest で今日すでに同じことが起きている。

### 16.12 未検証

- **実機の Zed。**組み込みの Go の `name` が `"Go"`
  （`crates/grammars/src/go/config.toml` の 1 行目。Rust 版は `name = "Rust"` で、
  いまの `extension.toml` の値と一致する）、LSP に送られる `languageId` が
  `LanguageName::lsp_id()` ＝ `"Plain Text"` 以外は `to_lowercase()` なので
  `"go"`——ここまでは zed main を取って確認した（2026-09-03）。**利用者が動かして
  いる版でそうか、Go のバッファで codegloss が起動するかは確かめていない。**
  同じファイルに `modeline_aliases = ["golang"]` があるが、`lsp_id()` が見るのは
  `name` なので送られてくるのは `"go"` である。
- **GOROOT は Go の標本ではない。**標準ライブラリは gofmt 済みで、コメントの
  書き方が揃っている。誤検出 1,208 行という数字はこの母集団に対するものである。
- **`_asm` のような生成器のファイルを除いていない。**上位 3 ファイルで誤検出の
  38% を占めるので、除けば数字は良くなる。除かなかったのは、利用者のリポジトリ
  にも同じものがあるからである。

## 17. JavaScript / TypeScript に対応する（Issue #30）

JSDoc は Javadoc と同じ `@tag` の形を使い、**後ろに来るものが違う**。Javadoc の
`@param id the user id` は引数名から始まり、JSDoc の
`@param {string} id The user id.` は型から始まる。1 行だけ見て見分けはつく——
中括弧があるかどうか——が、いまの `tag_lead` はタグ表の 1 つしか持っていないので、
**型が名前の枠に入り、名前が散文としてエンジンへ渡る**。

文法を配線するだけでは足りない、という点で #53（Rust の doctest）と
#30（Go の字下げ）と同じ形をしている。違うのは、JSDoc では**タグそのものが
「この後ろは散文ではない」と言っている**ことで、規則の置き場所はそこになる。

### 17.1 コーパスと道具

第三者のコメントはリポジトリに置けないので、コーパスは置く代わりに作れるように
してある（§12・§16 と同じ）。この節の数字は npm から取った 8 パッケージとその
依存で測った。

```sh
npm install lodash@4 date-fns@4 express@4 axios@1 zod@3 typescript@5 \
  yargs@17 commander@12
```

| コーパス | 母数 | ブロック |
|---|---|---|
| `.js` / `.mjs` / `.cjs` | 4,178 ファイル | 11,568 |
| `.d.ts` | 1,492 ファイル | 2,900 |
| `.ts`（`.d.ts` を除く） | 243 ファイル | 2,401 |
| 合計 | **5,913 ファイル** | **16,869** |

`*.min.js`・`dist/`・`umd/`・400 KB 超は外した。配布用の 1 行バンドルはエディタで
読まれるものではなく、コメントの母集団としても別物だからである。

ブロックは `cargo run -p codegloss-parser --example extract -- --lang javascript`
で誰でも取り出せる。

**タグの数字は 1 本のスキャンから取っている。**`/** ... */` のブロックだけを走り、
`docblock` と同じ順（開き・閉じ・星）でマーカーを落として `@` で始まる行を数えた
もので、パーサそのものではない。**ブロック数と「エンジンへ渡る断片の数」は出荷
経路（`extract_comment_blocks` と `GlossPlan::segments`）の実測**で、こちらは近似
ではない。2 つを混ぜないよう、以下では出所を書き分けてある。

### 17.2 型注釈が名前の枠を埋めていた

タグ行 18,891 のうち **3,518 行がタグの直後に `{型}` を書く**（スキャン）。
...
```
@param {string} id The user id.      → 断片 "id The user id."
@returns {Promise<User>} the user    → 断片 "{Promise<User>} the user"
@typedef {Object} User               → 断片 "{Object} User"
@type {import('./user').User}        → 断片 "{import('./user').User}"
```

1 行目は `TAGS_WITH_A_NAME` が最初の語を名前として取るので、`{string}` が名前の
枠に入り、**本当の名前 `id` が散文の先頭になる**。2 行目以降は型そのものが
エンジンへ渡る。

直し方は形の規則で足りる。タグの直後の**釣り合った中括弧**は型注釈で、行の lead
へ入れる。中括弧がどこで閉じるかは `preserve` の `braced_span` の答えをそのまま
使う——`{@code {1, 2}}` のために深さを数える関数がすでにあり、
`{Object.<string, {a: number}>}` に 2 つ目の答えを作らせないため。

`{@link User}` は型ではない。`{@` で始まるものは**どちらの流儀でも型ではない**ので、
そこだけ外してある（`preserve` のインラインタグの規則と食い違わせないため）。

### 17.3 タグごとに後ろに来るものが違う

型を lead へ入れただけでは `@throws` が壊れる。Javadoc は例外の型を裸の語で書き
（`@throws AuthenticationException if ...`）、JSDoc は中括弧で書く
（`@throws {Error} Thrown if ...`）。**同じ 1 つの枠**なので、型を取ったうえで
さらに語を取ると説明の先頭語（`Thrown`）を食う。

そこでタグ表を「後ろに何が来るか」の 4 種類にした（`TagArguments`）。

| 種別 | 後ろに来るもの | タグ |
|---|---|---|
| `Name` | （型）＋名前＋散文 | `@param` `@arg` `@argument` `@property` `@prop` `@typedef` `@callback` `@template` `@typeParam` |
| `TypeOrName` | 型 **または** 名前が 1 つ、その後ろが散文 | `@throws` `@exception` |
| `Opaque` | 散文は無い。行ごと写す | `@author` `@category` `@since` `@name` `@memberOf` ほか 13 |
| `Example` | 次のタグまでコード | `@example` |
| `Prose` | （型）＋散文。**既定** | 上に無いすべて |

既定を `Prose` にしてあるのは、聞いたことのないタグは何かを**説明している**方が
名前を書いているより確からしいからである。誤ると英語が原文のまま出る側に倒れる。

名前と説明のあいだの `- ` も lead へ入れる。JSDoc がそこを読み飛ばすからで、
入れないと `- The name of the user.` という**箇条書きの印から始まる文**を
エンジンに渡すことになる。コーパスで **1,990 行**がこの書き方をしている
（スキャン）。ハイフンの直後に空白が要る——`@returns -1 when absent` の `-1` は
区切りではなく符号である。

`Opaque` は数字が支えている。表に載せた 18 タグで **3,975 行**、うち
`@category` 1,535・`@name` 898・`@author` 724・`@memberOf` 410・`@since` 377 の
**5 つで 3,944 行**を占める（スキャン）。`@author Sasha Koss` を機械翻訳に通すのは
人名を別の人名にすることで、`@category Common Helpers` はページを束ねるラベルで
あって文章ではない。

残る 13 タグのうち姿を見せるのは 4 つだけで、合わせて 31 行しかない
（`@alias` 16・`@default` 7・`@license` 6・`@module` 2）。9 つ
（`@augments` `@borrows` `@copyright` `@extends` `@lends` `@memberof` `@mixes`
`@requires` `@version`）はこのコーパスに 1 行も無い。**数ではなく規則で載せて
ある**——JSDoc の説明が引数を名前と呼んでいるものだからである。

### 17.4 `@example` の中身はコード

`@example` は 1,574 回現れ、その後ろに **10,567 行**が続く（スキャン）。
...
**その行には「コードである」と言うものが何も無い。**フェンスは付いておらず
（`.js` の 1,192 例では **0 例**、`.d.ts` の 382 例のうち 22 例だけが本文の 1 行目に
フェンスを書く）、字下げも無い。
`CommentRules::Fenced` で読む限り、これは全部散文としてエンジンへ渡る——#53 と
#30 が消したはずの害そのものである。

言えるのはタグだけなので、タグに言わせた。**`@example` の本文は次のタグまでコード**
——JSDoc 自身の規則である。同じ規則の結果として、例の中で行頭に `@` を書くと
（TypeScript のデコレータなど）そこで例が終わる。JSDoc も同じところで終わるので、
挙動を合わせてある。

タグ行そのものに本文が書かれている例（`@example foo()`）は、コーパスに **0 件**
だった。

### 17.5 ツールへの指示を落とす

Go には `go/ast::isDirective` という規則が 1 つあった。JavaScript には無い。
ツールごとに綴りが違うので、こちらは**規則ではなく表**になる
（`ECMASCRIPT_PRAGMAS`）。

`@ts-` / `eslint-` / `prettier-ignore` / `biome-ignore` / `istanbul ignore` /
`c8 ignore` / `v8 ignore`、それと `<... />` の形（TypeScript の triple-slash
directive。TypeScript 自身の `tripleSlashXMLCommentStartRegEx` と同じく、タグ名
ではなく形で当てる）。

出荷経路の実測:

| コーパス | 規則なし | 規則あり | 落ちたブロック |
|---|---|---|---|
| `.js` | 11,765 | 11,568 | 197 |
| `.d.ts` | 3,050 | 2,900 | 150 |
| `.ts` | 2,539 | 2,401 | 138 |
| 合計 | 17,354 | 16,869 | **485（2.8%）** |

Go の 3.3% と同じ桁である。

**散文は 1 行も失われない。**規則あり・なしのブロックを両方吐いて突き合わせた。
規則なしの側にあって規則ありの側に**同じ文字列としては**無いブロックが 524 個
ある（実体が消えたものと、pragma のところで連なりが切れて短くなったものの合計。
純減は上の表の 485）。その 524 個の中にある pragma 以外の行は 84 行で、**そのうち
76 行は規則ありの側にも短いブロックの中に残っている**。どこにも残らずに消えるのは
8 行で、全部がコメントアウトされたコードの中に入れ子で書かれた
`// @ts-expect-error` である。

表の作り方で 1 つだけ気をつけたところがある。**`eslint` ではなく `eslint-` を
入れてある。**ESLint の指示は行コメントでは必ずハイフンが付く形
（`eslint-disable-next-line` ほか）で現れ、裸の `eslint` はブロックコメントの
インライン設定にしか使われない。`eslint` だけを見ると
`// eslint is configured in .eslintrc` が消える。同じ理由で ESLint の
`global` / `globals` / `exported` は表に入れていない——英語の単語が先だからである。

### 17.6 `/**` の星が本文の先頭に残っていた

これは配線しただけで出た欠陥で、JSDoc とは別の話である。

文法が doc コメントの印をノードで渡してくれるなら（Rust の `outer` / `inner`）、
本文の開始位置はその後ろになる。JavaScript と Go の文法は `(comment)` が 1 つ
あるだけで印を渡さないので、`/**` は「ブロックの開き `/*` ＋ 飾りの星」として
テキストに残り、**星が本文の先頭語になる**。

```
CommentBlock.text = "* Loads a user. @param {string} id The user id."
                     ^
```

`CommentBlock::text` は code lens の見出しに出るものであり、訳が無い間の hover
の中身でもある。**JSDoc ブロックのすべてに付く。**

doc の開きに名前は要らない。**ブロックの開きに継続行の印を 1 つ足したもの**で、
`CommentSyntax` はすでに両方を持っている。Rust は文法が印を渡すので通らず、
`/**/` のような空コメントも本文が空なので通らない。

### 17.7 効果

出荷経路の実測。`extract_comment_blocks` → `GlossPlan::segments()` を
`main` @ 0bd74fd と突き合わせたもので、両側ともディレクティブの規則は入れてある
（比べているのは `docblock` の変更だけ）。

| コーパス | 変更前 | 変更後 | 差 |
|---|---|---|---|
| `.js` | 29,806 | 22,623 | −7,183（−24.1%） |
| `.d.ts` | 10,702 | 9,235 | −1,467（−13.7%） |
| `.ts` | 2,878 | 2,874 | −4 |
| 合計 | **43,386** | **34,732** | **−8,654（−19.9%）** |

減った 8,654 はすべて型・名前・例の本文である。断片の中身で見ると:

| 断片 | 変更前 | 変更後 |
|---|---|---|
| 中括弧を含む（`{@` を除く） | 2,851 | 607 |
| `[` で始まる | 821 | 270 |

`.ts` がほとんど動かないのは母集団のせいで、この 243 ファイルは zod のテストが
大半を占め、そこにある `{` の多くは**コメントアウトされたテストコード**である。
JSDoc の問題ではなく、別の問題（コメントアウトされたコード）がそこにある。

### 17.8 Rust と Go は 1 バイトも動かない

`docblock` を触ったので、既存の 2 言語が動かないことを確かめてある。
`main` @ 0bd74fd と現在のツリーで、同じコーパスに同じ経路を通した出力を
バイト比較した。

| コーパス | 母数 | 比較したもの | 結果 |
|---|---|---|---|
| GOROOT（go1.24.7、`_test.go` を除く `.go`） | 5,517 ファイル | ブロック 204,105 / 断片 270,441 | **完全一致** |
| この機械の registry（`.rs`、400 KB 未満） | 8,596 ファイル | 断片 295,214 | **完全一致** |
| 自リポジトリ（`crates/**/*.rs`） | 40 ファイル | ブロック 1,461 / 断片 2,705 | **完全一致** |

**それでも `PIPELINE_VERSION` は上げてある（7 → 8）。**一致したのはこの母集団に
`@example` や `@author` を書いた Rust / Go のコメントが 1 つも無かったからで、
規則そのものは変わっている。上げなければ、そういうコメントを 1 つ持っている
利用者のディスクに古い訳が残り続ける。

### 17.9 払った代償

`@example` の本文にある **3,704 行の `//` コメント**は、もう訳されない。
date-fns はこう書く:

```js
 * @example
 * // Use the locale option to get the result in Spanish. Without setting it,
 * // the example would return "in 1 hour".
 * intlFormatDistance(new Date(1986, 3, 4, 11, 30, 0), ..., { locale: 'es' })
 * //=> 'dentro de 1 hora'
```

1 行目は読者に向けた英語で、訳されるべきものである。**4,155 行のコードを守る
代わりに、これを手放している。**

そうしたのは、**用例の中は写す**というのが CodeGloss がすでに取っている態度
だからである。Rust の doctest のフェンスの中にある `// コメント` も今日訳されて
いない。ここだけ別扱いにすると、同じ問いに 2 つの答えができる。「用例の中の
散文コメントを訳す」は `@example` とフェンスの両方に同時に効く 1 つの問いなので、
別 issue にしてある。

### 17.10 未検証

- **実機の Zed。**JavaScript / TypeScript / TSX は 3 つとも Zed 本体に組み込まれて
  いて（`crates/grammars/src/{javascript,typescript,tsx}/config.toml`）、`name` は
  `"JavaScript"` / `"TypeScript"` / `"TSX"`、LSP へ送られる `languageId` は
  `crates/language_core/src/language_name.rs` の `LanguageName::lsp_id()` ＝
  `"Plain Text"` 以外は `to_lowercase()` なので `"javascript"` / `"typescript"` /
  `"tsx"`——ここまでは zed main を取って確認した（2026-09-07）。**利用者が動かして
  いる版でそうか、実際にバッファで codegloss が起動するかは確かめていない。**
  欄は `docs/zed-display-notes.md` 2.6 にある。
- **npm のパッケージは JavaScript の標本ではない。**公開されているライブラリは
  JSDoc の書き方が揃っており、date-fns と lodash だけで `.js` の 86% を占める。
  アプリケーションのコードはタグが少なく pragma が多いはずで、17.5 の 2.8% は
  この母集団に対する数字である。
- **`.ts` の母数が小さい。**243 ファイルしかなく、しかも大半が 1 パッケージの
  テストである。`.d.ts` と `.js` の数字ほどの重みは無い。
- **訳文の質は測っていない。**この節が測っているのは「何がエンジンへ渡るか」で
  あって、渡ったものがどう訳されるかではない。JSDoc の英語に対する FuguMT の
  出来は §7 と同じ道具（`examples/probe.rs`）で測れるが、まだ測っていない。

## 18. 用例の中の散文コメントを訳す（Issue #70）

CodeGloss は「用例の中は写す」という態度を取ってきた。フェンスの中も、Go の
字下げされた行も、JSDoc の `@example` の本文も、まるごと原文のまま出す。
...
上の 2 行は、段落の散文と同じ書き手が同じ読者に向けて書いたものである。訳されて
いなかった理由は**コードの行に挟まれていたことだけ**だった。

これは `@example` だけの話ではない。#30 の JS / TS 対応で `@example` を写すように
したときに顕在化したが、Rust の doctest では最初からそうなっていた。**同じ問いに
2 つの答えを作らないため、両方を一度に直している。**

### 18.1 規模

`CommentShape` に単位ごとの標識を入れて再構成し、標識を持つ行を「訳される行」、
残りを「写される行」として数えた。**出荷経路そのもの**である（腕を組み直したもの
ではない）。

| コーパス | ファイル | ブロック | 写された `//` 行 |
|---|---|---|---|
| この機械の registry（Rust） | 8,596 | 243,920 | 9,013 |
| GOROOT（Go、go1.24.7） | 5,517 | 204,105 | 419 |
| npm の `.js` | 4,178 | 11,568 | 2,857 |
| npm の `.d.ts` | 1,492 | 2,900 | 868 |
| npm の `.ts` | 243 | 2,401 | 0 |
| **合計** | **20,026** | **464,894** | **13,157** |

**Rust が最大の母集団である**（9,013 行）。§17.9 で手放したと書いた JS の
3,704 行は、出荷経路で数え直すと 2,857 + 868 = 3,725 行だった（§17.9 はテキスト
スキャンの数字で、パーサそのものではない。§17.1 に書いた通りの差である）。

コーパスは §16.1（GOROOT）と §17.1（npm）と同じもの。

### 18.2 何を訳し、何を写すか

**用例の中の行のうち、行全体がコメントであるものを訳す。**マーカーと字下げは
lead に入るので、`    // seed the cache` の gloss は `    // キャッシュを温める`
になり、周りのコードのコメントとして読める。

マーカーは `LINE_MARKERS`（`///` / `//!` / `//`）。**囲っているコメントの語彙で
あって、用例の言語の語彙ではない。**それで足りるのは、`//` で開くコメントの中の
用例は `//` でコメントを書く言語で書かれているからである。同時にそれが限界でも
あって、`#` のコメントの中の Python の用例は #61 を先にやらないと見えない
——この module が知っているすべてのマーカーにすでに掛かっている制限と同じもの。

**3 つは散文ではない。**数字は「その規則が最初に弾いた行」で、順に見ていく。

| 規則 | 弾いた行 | 例 |
|---|---|---|
| マーカーの直後に空白が無い | **1,452** | `//=> Fri Aug 22 2014`、`////////` |
| 本文が `=>` で始まる | **844** | `// => false` |
| 英字が 2 つ続く箇所が無い | **375** | `// ...`、`// ]`、`// (0, 13)` |
| 合計 | **2,671** |

`=>` 以外の綴りは入れていない。`->`・`==>`・`⇒` はこの 4 コーパスに **0 件**で、
**根拠の無い規則はいつか文章を食う。**

空白の規則は代償を持つ。`//Call parse with the input to execute the parser`
のような、空白を空けずに書かれた散文が弾かれる（Rust の 204 行の一部）。それでも
入れてあるのは、買っているものの方が大きいからである。

### 18.3 連なりは 1 単位

**Rust の `//` 行の 49% は 2 行以上の連なりの中にある**（4,441 行 / 1,545 連なり）。
折り返した文の続きなので、1 行ずつエンジンへ渡すと**文の三分の一を渡して三分の一が
返ってくる**。

だから同じ lead の連なりは 1 単位にまとめ、**gloss では 1 行になる**。3 行に
折り返されていたコメントの gloss が 1 行なのは、gloss がコメントの訳であって
改行位置の写しではないからである。lead（字下げ＋マーカー）が違えば別のコメント
として扱う——書き手が同じ桁にマーカーを書き直したことが、続きである印である。

### 18.4 効果

| コーパス | 訳される行（前） | （後） | 差 | 写された `//` 行（前 → 後） |
|---|---|---|---|---|
| registry（Rust） | 245,581 | 251,695 | +6,114 | 9,013 → 520 |
| GOROOT（Go） | 205,592 | 205,824 | +232 | 419 → 96 |
| `.js` | 19,090 | 20,063 | +973 | 2,857 → 1,669 |
| `.d.ts` | 7,148 | 7,529 | +381 | 868 → 386 |
| `.ts` | 2,417 | 2,417 | 0 | 0 → 0 |
| **合計** | | | **+7,700** | **13,157 → 2,671** |

**10,486 行のコメントが訳されるようになり、それが 7,700 行の単位になる**
（差の 2,786 行は連なりがまとまったぶん）。

**コードの行は 1 行も動かない。**103,025（Rust）/ 16,985（Go）/ 3,366（`.js`）/
915（`.d.ts`）/ 37（`.ts`）は前後で完全に同じである。**コードが単位になっていない
ことが、この変更で最も確かめたかったことである**——#53 と #30 と §17.4 が消して
きた害を、逆向きに作り直していないという意味なので。

### 18.5 通り抜ける 1 家族

`=>` の値が複数行にまたがるとき、その途中の行は通る。

```js
 * //=> [
 * //   Sun Jul 02 1995 00:00:00,     ← 通る（Sun が英字 2 つ）
 * //   Wed Jan 01 2014 00:00:00,     ← 通る
 * // ]                               ← 英字が無いので弾かれる
```

**115 行**。全部 `.js` の 1 パッケージ（date-fns）の日付配列である。用例の中で
括弧の深さを数えれば捕まえられるが、そうすると**釣り合わない `(` を含む散文行が
その下の散文を飲み込む**ようになる。115 行のために作る規則ではないので、直さずに
ここに書いてある。

コメントアウトされたコード（`// info!(a = data; "…");`）も通る。これは**新しい
問題ではない**——用例の外の `// let x = 1;` は今日すでに訳されている。

### 18.6 `PIPELINE_VERSION`

8 → 9。`docblock` の出力が変わるので上げる。**この変更は §17 と違って、測った
コーパスの出力が実際に動く**（Rust だけで 6,114 行）。

### 18.7 未検証

- **gloss の見え方。**コードの中に日本語の `//` 行が混ざったとき、フェンスは
  Markdown のコードブロックとして描かれるので、日本語もそのブロックの中に入る
  はずである。**実機で確かめていない。**欄は `docs/zed-display-notes.md` 2.8。
- **連なりが 1 行になることの読み心地。**3 行の英語コメントが 1 行の日本語になると、
  その下のコードとの行対応がずれる。数えれば分かることではないので、欄にしてある。
