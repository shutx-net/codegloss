...
- `manifest.json` は**ファイルとしては照合されない**（自分のハッシュを自分では持てない）。ここでの担保は出どころ（HTTPS と、バイナリに焼き込んだ URL）だけ。
- **ただし `model_version` は突き合わせる。**`model_pack.rs` の `EXPECTED_MODEL_VERSION` と違うマニフェストは、**ほかのファイルを 1 つも取らずに**拒否される（`fetch`）。そしてその値は `convert.py` が**重みの SHA-256 の先頭 12 桁**から作っている。つまり、このビルドが受け取る別の重みを用意するには**先頭 48 ビットの部分プリイメージ**が要る。2^48 ≒ 2.8×10^14 は暗号の強度としては弱いが、**これはチェックサムではなく暗号学的性質**である——ここの SHA-256 は破損検出だけの役ではないので、「どうせチェックサムだから」と別の実装に取り替えてよいものではない。（`--model-pack` で手元のディレクトリを直に指したときは、この突き合わせを通らない。「上げる前に確かめる」を `--fetch-model` で通すのはそのため。）
- **とはいえ、URL を握った相手は manifest の残りの欄も握る。**`files` の SHA-256 は manifest 由来なので、重みを差し替える側は差し替えたもののダイジェストを書けばよい。48 ビットが縛っているのは `model_version`——**この版のビルドが受け取るパックの同一性**であって、配布元の真正性ではない。**信頼の起点は出どころのまま。**
- **manifest 以外のファイルはすべて検証される**（バイト数と SHA-256。manifest が 1 つもファイルを挙げていない場合もそこで落ちる）。途中で切れたダウンロードや、殺されたプロセスが半分書いたパックを捕まえるため。**壊れた重みはエラーにならず、流暢な出鱈目になる**ので、使う前に捕まえる必要がある。
